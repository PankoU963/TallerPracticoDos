...
public class Clean1 : MonoBehaviour
{
    ...
    // Hover / input settings
    [Header("Input Settings")]
    [SerializeField] private bool _paintOnHover = true; // if true, painting happens when the mouse pointer passes over the surface
    [SerializeField] private bool _requireMouseMoveForHover = true; // if true, only paint on hover when the mouse moved since last frame
    ...
    // Track last mouse position so hover painting can optionally require movement
    ...
    private void Update()
    {
        ...
        // Mouse input: either paint while holding button (legacy) or paint on hover if enabled
        if (_paintOnHover)
        {
            // Optionally only trigger when mouse moved to avoid repeated paints while stationary
            if (!(_requireMouseMoveForHover && Input.mousePosition == _lastMousePosition))
            {
                ProcessPointer(Input.mousePosition);
            }
            _lastMousePosition = Input.mousePosition;
            // do not return; touches are still processed below
        }
